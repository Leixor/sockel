...
export class Sockel<CustomUser extends { id: string }> extends WebSocket.Server {
    private constructor(
        ...
        this.on("connection", (ws, request: http.IncomingMessage) => {
            const user: CustomUser = extractUserFromRequest(request);

            const connectedUser: ConnectedUser<CustomUser> = { ws, user };
